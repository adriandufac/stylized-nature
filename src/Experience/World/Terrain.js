...
export default class Terrain extends WorldComponent {
  ...
  setDebug() {
    ...
    meshFolder
      .add(this.params, "size", 10, 200, 1)
      .name("Taille")
      .onFinishChange(() => this.rebuild());
    meshFolder
      .add(this.params, "segments", 16, 512, 1)
      .name("Subdivisions")
      .onFinishChange(() => this.rebuild());
    ...
    this.params.octaves.forEach((octave, i) => {
      ...
      octaveFolder
        .add(octave, "frequency", 0.001, 1, 0.001)
        .name("Fréquence")
        .onChange(() => this.applyHeights())
        .onFinishChange(() => this.trigger("resampled"));
      octaveFolder
        .add(octave, "amplitude", 0, 10, 0.05)
        .name("Amplitude")
        .onChange(() => this.applyHeights())
        .onFinishChange(() => this.trigger("resampled"));
    });
    ...
    const seedCtrl = folder
      .add(this.params, "seed")
      .name("Seed")
      .onFinishChange(() => this.applySeed());
